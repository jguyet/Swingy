...
public class Sirene extends Entity {
	private static Sprite		sprite = Sprite.SIRENE;
	private static ArrayList<Artefact> drops = new ArrayList<Artefact>();
	
	static {
		drops.add(new Armor("SireneAmor", 10, new Statistics(EStatElement.Defense, 9), false, 397));
		drops.add(new Helm("SireneHelm", 15, new Statistics(EStatElement.HitPoint, 50), false, 369));
		drops.add(new Weapon("SireneSword", 15, new Statistics(EStatElement.Attack, 30), false, 368));
	}
	
	public Sirene(String name, Statistics stats, Vector2 position) {
